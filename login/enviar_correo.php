...
if (!isset($_POST['email'])&&!isset($_POST['dni'])) {
    ...
    if (password_verify($r_b_sesion['token'], $token)) {
        $id_docente = buscar_usuario_sesion($conexion, $id_sesion, $token);
        $enviar = 1;
    } else {
        $enviar = 0;
    }
    ...
    $busc_user = "SELECT * FROM usuarios WHERE dni='$dni' AND correo='$correo'";
...
if ($enviar) {
    ...
    $b_docente = buscarUsuarioById($conexion, $id_docente);
